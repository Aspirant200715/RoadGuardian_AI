          ...
          return (
            <Card key={user.id} className={`rounded-sm shadow-sm bg-white dark:bg-card ${isTop ? 'md:-translate-y-2 border-t-4 border-t-[#FF9933]' : 'border-t-4 border-t-[#000080] dark:border-t-primary'}`}>
              ...
              <CardContent className="flex flex-col items-center p-6 text-center space-y-4">
                ...
                <table className="w-full text-left text-[10px] font-mono border border-border">
                  <tbody className="divide-y divide-border bg-slate-50 dark:bg-muted/30">
                    <tr>
                      <td className="py-1 px-3 font-bold text-muted-foreground uppercase w-1/2">Verified Logs</td>
                      <td className="py-1 px-3 font-black text-[#138808]">{user.verified_reports || 0}</td>
                    </tr>
                    <tr>
                      <td className="py-1 px-3 font-bold text-muted-foreground uppercase w-1/2">Pending Logs</td>
                      <td className="py-1 px-3 font-black text-[#FF9933]">{user.pending_reports || 0}</td>
                    </tr>
                    <tr>
                      <td className="py-1 px-3 font-bold text-muted-foreground uppercase">Civic Points</td>
                      <td className="py-1 px-3 font-black text-[#000080] dark:text-primary">{user.points}</td>
                    </tr>
                  </tbody>
                </table>
              </CardContent>
          ...
          <table className="w-full text-sm text-left border-collapse">
            <thead className="text-[10px] text-muted-foreground uppercase tracking-widest bg-slate-50 dark:bg-muted/50 border-b border-border">
              <tr>
                ...
                <th className="px-6 py-3 font-bold text-right border-r border-border">Pending Reports</th>
            ...
            <tbody className="divide-y divide-border">
              {leaderboard.slice(3).map((user, idx) => (
                <tr key={user.id} className="bg-white dark:bg-card hover:bg-slate-50 dark:hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-6 py-3 text-right font-mono font-medium text-[#138808] border-r border-border">
                    {user.verified_reports || 0}
                  </td>
                  <td className="px-6 py-3 text-right font-mono font-medium text-[#FF9933] border-r border-border">
                    {user.pending_reports || 0}
                  </td>
                  <td className="px-6 py-3 text-right font-mono font-black text-[#000080] dark:text-primary">
                    {user.points}
                  </td>
                </tr>
              ))}
