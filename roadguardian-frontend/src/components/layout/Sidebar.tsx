...
export const Sidebar = () => {
  ...
  const links = [
    ...
    { to: '/heatmap', icon: Map, label: 'Live Hazard Map' },
  ...
  links.push({ to: '/whatsapp-demo', icon: MessageCircle, label: 'WhatsApp Sandbox' });
  links.push({ to: '/whatsapp-messages', icon: MessageCircle, label: 'WhatsApp Comm Center' });
  ...
  return (
    <>
      ...
      <aside className={cn(
        "fixed md:static z-40 h-full w-[260px] border-r border-border bg-card transition-transform md:translate-x-0 shadow-md shrink-0 flex flex-col",
