...
class HazardService:
    ...
    async def create_hazard(
        ...
        department_mapping = {
            "Water & Sanitation Board": ["waterlogging", "manhole_defect"],
            "Road Department": ["pothole", "crack", "missing_signs", "broken_dividers", "pavement_defect", "road_debris", "construction_hazard"],
            "Forestry Department": ["fallen_tree"],
            "Power Department": ["electrical_hazard", "street_light_fault"],
            "Traffic Police": ["traffic_signal_fault"],
            "Animal Control": ["animal_carcass"]
        }
        
        linked_department = "Municipal Corporation"
        for dept, hazards in department_mapping.items():
            if hazard_type in hazards:
                linked_department = dept
                break
        
        # Fallback keyword analysis if still "other"
        if linked_department == "Municipal Corporation" and description:
        ...
        # Award 50 base points to the user for submitting a hazard report
        user_stmt = select(User).where(User.id == user_id)
        user_res = await db.execute(user_stmt)
        submitting_user = user_res.scalar_one_or_none()
        if submitting_user:
            submitting_user.points += 50
